...
class HplusAnalyzer_kinfit11_2M_v1 : public ObjectSelector, HistogramPlotter
{
public :
  HplusAnalyzer_kinfit11_2M_v1() : ObjectSelector(), HistogramPlotter()
  {
    DRMIN_JET = 0.5;
    DRMIN_ELE = 0.5;
    METCUT_   = 30.0;
    ...
    LumiWeights_ = reweight::LumiReWeighting("MC_Pileup_Summer2012_600bins.root","Data_Pileup_2012_ReRecoPixel-600bins.root", "pileup", "pileup");
...
void HplusAnalyzer_kinfit11_2M_v1::CutFlowAnalysis(TString url, string myKey, bool isData, string evtType, string Cat){
  
  //  gSystem->Exec("mkdir 8TeV/kinfit4_v10/");
...
void HplusAnalyzer_kinfit11_2M_v1::CutFlowProcessor(TString url,  string myKey, TString cutflowType, bool isData, TFile *outFile_, string Cat){
    int input_count = 0;
  ...
  for(int i=0; i<nEntries; ++i){
    ...
    if(!isData){
      ...
      if(sampleName.find("TTBAR") != string::npos || sampleName.find("WH") != string::npos || sampleName.find("HH") != string::npos){
    ...
    */
    ...
    for(size_t ijet = 0; ijet < j_final.size(); ijet++){
      int ind_jet = j_final[ijet];
      if(pfJets[ind_jet].bDiscriminator["combinedSecondaryVertexBJetTags"] > 0.244)count_CSVL++;
...
void HplusAnalyzer_kinfit11_2M_v1::CreateAnalHistos(TString cutflowType, TFile* outFile_)
{
  ...
  addHisto("final_multi_jet", cutflowType, 10,0,10);
  addHisto("CSVL_count", cutflowType, 10,0,10);
  ...
  addHisto("deltaR_match_jet12", cutflowType, 100, 0.0, 10.);
  // mis match
  ...
  addHisto("deltaR_mismatch_jet12", cutflowType, 100, 0.0, 10.);
  ...
  addHisto("chi2_fit", cutflowType, 900, 0.0, 9.0);
  ...
  addHisto("final_multi_jet", cutflowType+"/BTag", 10,0,10);
  addHisto("CSVL_count", cutflowType+"/BTag", 10,0,10);
  addHisto("CSVM_count", cutflowType+"/BTag", 10,0,10);
  addHisto("wmt", cutflowType+"/BTag", 100, 0., 200.);
...
float HplusAnalyzer_kinfit11_2M_v1::reweightHEPNUPWJets(int hepNUP) {
...
float HplusAnalyzer_kinfit11_2M_v1::reweightHEPNUPDYJets(int hepNUP){
